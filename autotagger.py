...
class Autotagger:
    ...
    def predict(
        ...
        for filename, image in zip(filenames, images):
            ...
            ratings = (
                pd.DataFrame(
                    [("rating:" + labels[i][0], labels[i][1]) for i in self.rating_indices], columns=["tag", "score"]
                )
                .sort_values("score", ascending=False)
                .head(1)
            )
            gentags = pd.DataFrame(
                [x for x in [labels[i] for i in self.general_indices] if x[1] > general_threshold],
                columns=["tag", "score"],
            ).sort_values("score", ascending=False)
            chartags = pd.DataFrame(
                [x for x in [labels[i] for i in self.character_indices] if x[1] > character_threshold],
                columns=["tag", "score"],
            ).sort_values("score", ascending=False)
